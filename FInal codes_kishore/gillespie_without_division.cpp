...
int main()
{
	...
	FILE *output, *parameters, *avg_file, *dist;// define the files
	float avg[121000][7] = {0};
	int i,conc[7],j,k, count,z,distr[1000][7]={0};// define integer variables: conc[] for concentrations, i,j and k loop variables
	...
	while (time < 120000)
	{
		...
		if (z >= count)
		{ 
			fprintf(output, "%d %d %d %d %d %d %d %d\n", (int)(time), conc[0], conc[1], conc[2], conc[3], conc[4], conc[5], conc[6]);			
			count += 1;
			for(j=0;j<7;j++)
				avg[z][j] += conc[j];
		}
	...
	avg_file = fopen("avg.txt","w");
	for(i=0;i<20000;i++){
		fprintf(avg_file, "%d",i*6);
		for(j=0;j<7;j++)
			fprintf(avg_file, " %f", avg[i][j]/1000);
		fprintf(avg_file, "\n");
	}
	fclose(avg_file);

	dist = fopen("distribution.txt","w");
	for(i=0;i<1000;i++)
	{
		fprintf(dist, "%d",i);
		for(j=0; j<7; j++)
			fprintf(dist, " %d", distr[i][j]);
		fprintf(dist, "\n");
	}
